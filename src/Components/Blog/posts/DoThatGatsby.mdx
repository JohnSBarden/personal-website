...
I wrote this page originally in some basic React.  I did the normal things like bootstrapping from create-react-app, and bootstrapping with React-Bootstrap for some basic formatting.  After customization and personalizing everything, I thought about what the meat of the site would be.  A quick blog space seemed like a good way to add content without having to be original, so let's do it.
...
Who chose such an easy to mistype name?  I hit Ga_s_tby every time without fail.  At least I can rebrand it as `yarn develop`, I guess.
